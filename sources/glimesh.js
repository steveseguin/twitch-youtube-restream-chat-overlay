...
$("body").unbind("click").on("click", ".bubble", function () { //  #chat .chat-messages .bubble

 
  $(".hl-c-cont").remove();
  var chatname = $(this).find(".user-message-header > a").text();

  if (showOnlyFirstName) {
    chatname = chatname.replace(/ .*/,'');
  }
  
  var chatmessage = $(this).find('.user-message').html();
...
$("body").on("click", ".btn-clear-glimesh", function () {
  pushMessage(false);
...
onElementInsertedTwitch("#chat-messages", "user-message", function(element){
